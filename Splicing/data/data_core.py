...
from Splicing.data.dataset_Taobao import Taobao
...
class SplicingDataset(Dataset):
    def __init__(self, crop_size, grid_crop, blocks=('RGB',), mode="train", DCT_channels=3, read_from_jpeg=False, class_weight=None):
        ...
        if mode == "train":
            ...
            self.dataset_list.append(Taobao(crop_size, grid_crop, blocks, DCT_channels, 'Taobao_train_list.txt'))
        elif mode == "valid":
            ...
            self.dataset_list.append(Taobao(crop_size, grid_crop, blocks, DCT_channels, 'Taobao_valid_list.txt'))
        elif mode == "arbitrary":
        ...
        self.smallest = min([len(lst) for lst in self.dataset_list])  # smallest dataset size (IMD:1869)
